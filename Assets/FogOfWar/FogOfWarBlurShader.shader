	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed3 frag (v2f i) : SV_Target
			{
				...
				#define GETPIXEL(xx, yy) tex2D(_MainTex, i.uv + float2(pixelsize.x * xx, pixelsize.y * yy)).rg
				#define CONVOLUTION(xx, yy, m) GETPIXEL(xx, yy) * m
					...
					float2 color = CONVOLUTION1(-1, 1, 2, 1);
					...
					float2 color = CONVOLUTION2(-2, 1, 4, 6, 4, 1);
					...
					// find edge
					float2 fog11 = GETPIXEL(-1, -1);
					float2 fog21 = GETPIXEL(0, -1);
					float2 fog31 = GETPIXEL(1, -1);
					float2 fog12 = GETPIXEL(-1, 0);
					float2 fog22 = GETPIXEL(0, 0);
					float2 fog32 = GETPIXEL(1, 0);
					float2 fog13 = GETPIXEL(-1, 1);
					float2 fog23 = GETPIXEL(0, 1);
					float2 fog33 = GETPIXEL(1, 1);

					float2 edge = fog11 + fog12 + fog13 + fog21 + fog23 + fog31 + fog32 + fog33;
					edge = -edge + fog22 * 8;

					// find blur
					float2 blur = (fog11 + fog13 + fog31 + fog33) + (fog12 + fog21 + fog32 + fog23) * 2 + fog22 * 4;
					blur *= 1.0f / 16;

					// antialias
					//float color = saturate(edge);
					float2 color = lerp(fog22, blur, saturate(edge));
				#endif

				return fixed3(color.rg, 0);
			}
			ENDCG
		}
	}
