...
namespace FoW
{
    public class FogOfWarDrawerSoftware : FogOfWarDrawer
    {
        ...
        {
            byte[] target = type == FogOfWarValueType.Visible ? _fogValuesVisible : _fogValuesPartial;
            System.Array.Copy(values, target, target.Length);
        }

        void Unfog(int x, int y, byte v)
        {
            int index = y * _map.resolution.x + x;
            if (_fogValuesVisible[index] > v)
                _fogValuesVisible[index] = v;
        }
            ...
            for (int y = info.yMin; y <= info.yMax; ++y)
            {
                for (int x = info.xMin; x <= info.xMax; ++x)
                {
                    ...
                    if (drawtexture)
                    {
                        ...
                        Unfog(x, y, shape.SampleTexture(u, v));
                    }
                    else
        ...
        void DrawRotatedBox(FogOfWarShapeBox shape)
        {
            ...
            for (int y = info.yMin; y < info.yMax; ++y)
            {
                ...
                for (int x = info.xMin; x < info.xMax; ++x)
                {
                    ...
                    // unfog
                    if (drawtexture)
                        Unfog(x, y, shape.SampleTexture(0.5f + u * invfogsize.x, 0.5f + v * invfogsize.y));
                    else
                        Unfog(x, y, brightness);
                }
            }
